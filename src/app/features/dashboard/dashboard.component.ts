...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    const dateList = this.dashboardStore.pipe(select(dashboardSelectors.dateList));
    const groupingsByDate = this.sharedStore.pipe(select(sharedSelectors.historyGroupingsByDate));
    const groupingsByPlatform = this.sharedStore.pipe(select(sharedSelectors.historyGroupingsByPlatform));
    const groupingsByGame = this.sharedStore.pipe(select(sharedSelectors.historyGroupingsByGame));
    ...
    this.dateRangeType$ = this.dashboardStore.pipe(select(dashboardSelectors.rangeType));
  }
}
